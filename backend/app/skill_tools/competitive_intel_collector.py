"""Deterministic USAspending collector for competitive-intel (Workflow B/C).
...
_USASPENDING_SERVER = "usaspending-gov-mcp"
_LOOKUP_LIMIT = 10
...
async def tool_collect_competitive_obligation_intel(
    ...
    summary = {
        "artifact_path": artifact_result_payload["path"],
        "scope": effective_scope,
...
async def _collect_parent_vehicle_orders(
    ...
    cap = _pagination().max_orders_per_vehicle
    if len(orders) > cap:
        warnings.append(
            f"Vehicle has {len(orders)} child orders; processing first {cap}. "
            "Raise max_orders_per_vehicle in Settings → Skill runtime caps if data is missing."
        )
        orders = orders[:cap]
...
async def _fetch_idv_children(
    ...
    while True:
        try:
            payload = await _call_usaspending_json(
                ...
                "get_idv_children",
                {
                    ...
                    "limit": _pagination().idv_page_limit,
                    "page": page,
                },
                tools_invoked,
            )
        ...
        if not _page_has_next(payload, len(batch), _pagination().idv_page_limit):
            return results
        if not batch:
            warnings.append(
            ...
            return results
        if page >= _pagination().max_idv_pages:
            warnings.append(
                f"IDV child traversal hit max_idv_pages={_pagination().max_idv_pages}; "
                f"raise SKILL_COLLECTOR_MAX_IDV_PAGES in Settings if orders are missing."
            )
            return results
        page += 1
...
async def _fetch_idv_activity(
    ...
    while True:
        ...
        if page >= _pagination().max_idv_pages:
            warnings.append(
                f"IDV activity traversal hit max_idv_pages={_pagination().max_idv_pages}; "
                "raise SKILL_COLLECTOR_MAX_IDV_PAGES in Settings if orders are missing."
            )
            return results
        page += 1
...
async def _fetch_transactions(
    ...
    *,
    ...
    results: list[dict[str, Any]] = []
    for page in range(1, _pagination().max_transaction_pages + 1):
        try:
            payload = await _call_usaspending_json(
                session,
                "get_transactions",
                {
                    ...
                    "limit": _pagination().transaction_page_limit,
                    "page": page,
                    "sort": "action_date",
                    "order": "asc",
                },
        ...
        if not _page_has_next(payload, len(batch), _pagination().transaction_page_limit):
            return results

    warnings.append(
        f"Transaction traversal hit the internal safety ceiling of {_pagination().max_transaction_pages} pages for {award_piid or award_id}."
    )
    return results
