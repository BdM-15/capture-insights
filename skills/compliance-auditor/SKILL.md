...
- **Run** from Agent Skills or chat ("audit FAR clauses", "compliance audit").
- Scans pursuit Studio markdown for FAR/DFARS refs and `shall` language.
- Validates clause numbers via **eCFR MCP** when live MCPs are enabled.
- Writes `04_proposal/compliance_audit.json` + `.md` under the active pursuit slug.
- Evidence cites **vault file paths** — not `entity_id` / `chunk_id`.

Vendored from Theseus; full multi-turn checklist below is the agent refinement target. The backend runner delivers the deterministic C4/C9/C10 slice today.

You are a senior compliance reviewer working against the active **pursuit workspace** (vault markdown files). Produce a **gap report** that a Capture Manager and Contracts SME can act on within an hour. Every finding must cite a real **file path** and quoted excerpt from pursuit or vault sources.
