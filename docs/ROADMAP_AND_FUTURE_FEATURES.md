...
Current phase (as of June 2026): **Two independent foundations first** — (A) USASpending dashboard fully functional from DuckDB bulk history, (B) foundational global wiki browse/read/Obsidian in `data/knowledge/global/`. Phase 3 vault lint/index + handoff complete. **Cross-seed** (dashboard data → auto-written global pages via LLM) is explicitly **future** — only after A and B are verified solid. Brain/ stays data-tied via +brain. Pursuits/wiki later.

Previous phase recap in "Recently Completed". Focus now: make the native .md vault (brain/ + global/ + pursuits/ + education/ + schema/) production-ready for heavy use in Obsidian desktop + external agents (via obsidian-skills) + the app's local LLM. Add structural lint + index maintenance helpers so the "programmer" (LLM) can keep the "codebase" (wiki) healthy without manual drudgery. Handoff docs so you (or Claude/Cursor/etc.) can drop in and start enhancing immediately. Small, focused, no bloat. UI changes minimal (vault view already good enough).
...
- **Phase 2 complete — Vault UI/UX + theme consistency ("much better... on the right track")**: Deep Theseus-inspired but lean reusable architecture: .action-btn (single module + .pipeline/.vault semantic variants), .island (purposeful separated containers with accent, hover lift/glow, breathing), .section-head, .entry (uniform list rows). Vault view rebuilt into clean islands with correct groupings (narrative+access together full-width + education, data-seed block separate + contextual, prominent copy-full + your overlay in entries, native disk truth, light guidance). Every action has descriptive label + rich title (purpose, outcome, provenance, how it compounds, Obsidian use). Whole app now feels connected (vault no longer random/tacked-on). User sign-off: "That is looking much better. Not perfect, but on the right track. Good enough for now. move to phase 3".
...
- Brain/Wiki accumulator as first-class counterpart to Pipeline (directly supports "if we come across a new competitor we can click that button and it would run like a competitive intel research or add to an existing wiki"). Current simple JSON + citations is the foundation; evolving to native Obsidian/Karpathy LLM wiki (plain .md files with [[wikilinks]], LLM synthesis seeded from data insights, structure ready to grow to global_wiki/ + pursuits tiers) per user confirmation — light My Focus on current first, then wiki foundation slice.
- Wiki foundation polish (after initial .md write + "continue building based on your recommendations"): robust list_brain_wiki_files (frontmatter type priority + legacy dir "agencys/" -> "agencies/" migration + clean excerpts), primary Brain list in sidebar now pulls "Synthesized wiki note" excerpt + path from the matching native .md (primary accumulator view becomes the rich LLM/Obsidian one), Wiki entries list gets copy-path buttons, light global/ + pursuits/ structure placeholders + READMEs added in _ensure_parent (non-breaking, explains the ariadne-inspired growth path). End-to-end verified (add item, list shows correct types/excerpts, dirs present, UI renders richer content). All small/reuse/citations/per-tab.
...
- **Knowledge Layer / Structured Wiki Vault** — The Brain/Wiki accumulator evolves to native Obsidian/Karpathy LLM wiki (plain .md, [[wikilinks]], LLM-synthesized entries from USASpending data + citations, compounds). Current JSON is transitional; foundation slice writes files to data/knowledge/brain/ so user can use Obsidian immediately. Will expand to match ariadne reference (global_wiki, competitor_intel, pursuits/wiki) when ready. (See Ariadne "Knowledge Layer" mapping in README and session plan for sequence.)
